...
ll recur(int idxx,int idxy){
    if(idxy==n)return 0;
    if(dp[idxx][idxy]!=-1)return dp[idxx][idxy];
    return dp[idxx][idxy]=max(arr[idxx][idxy]+recur(idxx^1,idxy+1),recur(idxx,idxy+1));
}
