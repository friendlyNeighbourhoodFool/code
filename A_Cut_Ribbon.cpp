...
ll solve(ll len)
{
    ...
    if (len >= a && len >= b && len >= c)
        return dp[len]=max(1 + solve(len - a), max(1 + solve(len - b), 1 + solve(len - c)));
    else if (len >= a && len >= b)
        return dp[len]=max(1 + solve(len - a), 1 + solve(len - b));
    else if (len >= b && len >= c)
        return dp[len]=max(1 + solve(len - b), 1 + solve(len - c));
    else if (len >= a && len >= c)
        return dp[len]=max(1 + solve(len - a), 1 + solve(len - c));
    else if (len >= a)
        return dp[len]=1 + solve(len - a);
    else if (len >= b)
        return dp[len]=1 + solve(len - b);
    else if (len >= c)
        return dp[len]=1 + solve(len - c);
    else
        return dp[len]=INT_MIN;
}
